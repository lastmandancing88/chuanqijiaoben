...
{
    public class DM : IDisposable
    {
        #region import DLL 函数
        [DllImport("dm.dll", CharSet = CharSet.Unicode, CallingConvention = CallingConvention.StdCall)]
        ...
        public DM(string path = "dm.dll")
        {
            instance = CreateDM(path);
        }
        private DM()
        {
            try
